...
future<void> session(shared_ptr<ip::tcp::socket> s, size_t block_size)
{
    ...
    for (;;)
    {
        auto n = co_await async_read_some(*s, buffer(buf_.data(), block_size), my_use_future);
        co_await async_write(*s, buffer(buf_.data(), n), my_use_future);
    }
}
...
int main(int argc, const char* argv[])
{
    ...
    // default args
    static const char* defargs[] = {"easy", "0.0.0.0", "8888", "128"};
