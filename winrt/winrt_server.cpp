...
int wmain(int argc, wchar_t* argv[])
{
    ...
    try
    {
        ...
        if (argc != 4)
        {
            wcerr << L"Usage: winrt_server <port> <threads> <blocksize>" << endl;
            return 1;
        }

        wstring_view service_name = argv[1];
        uint32_t thread_count = stoi(argv[2]);
